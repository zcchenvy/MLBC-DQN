...
class Net(nn.Module):
    ...
    def __init__(self, ad_dim):
        ...
        self.fc1 = nn.Linear(ad_dim*4 + 1, ad_dim * 16)
...
def test():
    ...
    dqn.eval_net[0].load_state_dict(torch.load('eval_net_6800.pkl'))
    [W, w, v, vw, t], done = env[layer].reset()
    state = trans_state(W, w, v, vw, t)
    ep_reward = 0
    ...
    while not done:
        ...
        action = dqn.choose_action(state, layer, env[layer].reset_W, way='test')
        # action = np.argmax(vw)
        ...
        if done:
            ...
            env[layer].bit_list.append(1)
            bits = list_split(env[layer].bit_list, 10)
            plt.plot(np.sum(bits,axis=1))
            plt.show()
            break
        state = next_state
        W, w, v, vw, t = W_, w_, v_, vw_, t_
